...
from lib.utils import conv, relu, interpolate, adaptive_cat, MultiReceptiveConv
...
class Translate_F(nn.Module):

    def __init__(self, ic, mid, oc, use_bn=False):
        super().__init__()
...
class SegNetwork(nn.Module):
    ...
    def forward(self, scores, features, gate, prevX, Diff, image_size, org_score=None):
        '''
        scores : b * 1 * 30 * 54
        ...
        x = TransprevX*(gate.unsqueeze(2).unsqueeze(2).expand_as(prevX)) + \
            curr_x*(1-gate).unsqueeze(2).unsqueeze(2).expand_as(curr_x)
        ...
        new_prevX = prevX * (1 - gate.unsqueeze(2).unsqueeze(2).expand_as(prevX)) + \
                    curr_x * gate.unsqueeze(2).unsqueeze(2).expand_as(curr_x)

        return x_proj,  new_prevX

    def Test_Forward(self, scores, features, gate, prevX, Diff, image_size):
        num_targets = scores.shape[0]
    ...
    def Test_Init(self, scores, features):
        num_targets = scores.shape[0]
        num_fmaps = features[next(iter(self.ft_channels))].shape[0]
        if num_targets > num_fmaps:
            multi_targets = True
